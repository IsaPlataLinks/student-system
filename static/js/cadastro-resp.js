...
document.addEventListener('DOMContentLoaded', function() {
    // Máscara WhatsApp do Responsável
    const inputWhatsApp = document.querySelector('input[name="whatsapp_responsavel"]');
    if (inputWhatsApp) {
    ...
    // Máscara CEP do Responsável
    const inputCEP = document.querySelector('input[name="cep_responsavel"]');
    if (inputCEP) {
        ...
        inputCEP.addEventListener('blur', async function() {
            ...
            if (cep.length === 8) {
                try {
                    ...
                    if (!dados.erro) {
                        const inputEndereco = document.querySelector('input[name="endereco_responsavel"]');
...
formResponsavel.addEventListener('submit', async function(e) {
    e.preventDefault();

    // Validações
    const emailResp = document.querySelector('input[name="email_responsavel"]').value;
    const whatsappResp = document.querySelector('input[name="whatsapp_responsavel"]').value;
    const cepResp = document.querySelector('input[name="cep_responsavel"]').value;

    if (!validarEmail(emailResp)) {
        mostrarAlerta('E-mail do responsável inválido! Verifique o formato.', 'danger');
        return;
    }

    if (!validarWhatsApp(whatsappResp)) {
        mostrarAlerta('WhatsApp inválido! Use o formato (XX) XXXXX-XXXX', 'danger');
        return;
    }

    if (!validarCEP(cepResp)) {
    ...
    try {
        ...
        // Enviar para API
        const response = await fetch(`${API_URL}/cadastro/responsavel`, {
            method: 'POST',
            body: formData
        });
        ...
        if (response.ok) {
            ...
            localStorage.setItem('cadastroSucesso', JSON.stringify({
                tipo: 'responsavel',
                nome: formData.get('nome_responsavel'),
                nomeAluno: formData.get('nome_aluno'),
                id: resultado.id
            }));
